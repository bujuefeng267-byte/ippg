...
class Session:
    ...
    def _publish(self, event):
        ...
        with self.lock:
            ...
            event['input_queue_lag_s'] = max(0., (self.last_time or 0.)-event['window_end_s'])
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            if self.path == '/':
                ...
                body = (HERE/'index.html').read_text(encoding='utf-8').replace('__TOKEN__', service.token).encode()
                ...
                self.reply(service.current.status() if service.current else {'active':False})
            else:
        ...
        def do_POST(self):
            ...
            try:
                ...
                if self.path == '/api/frame':
                    ...
                    if frame is None or frame.size > 12_000_000:
                        raise ValueError('Expected a camera JPEG smaller than four megapixels')
...
def main():
    ...
    print(f'Open http://localhost:{args.port} in a desktop browser. Ctrl+C stops the service.',flush=True)
    try:
