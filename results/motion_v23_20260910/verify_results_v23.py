...
def main():
    ...
    assert tests['passed'] and tests['run']>=73 and tests['errors']==tests['failures']==tests['skipped']==0
    for n,h in tests['test_hashes'].items():assert sha(HERE/n)==h
    for n,h in report['source_protocol_v23']['source_hashes'].items():assert sha(HERE/n)==h
