#!/usr/bin/env python3
"""V2.4 bounded paired-pixel reconstruction and guarded waveform fusion.
...
def main():
    ...
    if args.algorithm_mode=='guarded_fusion':
        ...
        proposals,wave,diagnostics,routing,branches=guarded_fuse(original_signals,signals,trace,fps,
            args.window,args.step,args.min_bpm,args.max_bpm,config=config)
    ...
    summary=dict(version='multi_roi_v2.4_experimental',identity=identity,source_hashes=frozen_sources,fps=fps,frames=len(trace),
    ...
    summary['guard_config']=asdict(GuardConfig()) if args.algorithm_mode=='guarded_fusion' else None
    if args.algorithm_mode=='guarded_fusion':
    ...
    axes[0].set(ylabel='Sampling',ylim=(-.1,1.2),title=f'V2.4 {args.algorithm_mode}: sampling and offline HR')
