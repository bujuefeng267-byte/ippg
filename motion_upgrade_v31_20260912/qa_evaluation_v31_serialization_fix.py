"""Serialization-only repair of frozen CSV-only V31 audit; calculations unchanged.

The frozen QA completed every assertion but numpy.bool_ failed in json.dump.
Its original source and incomplete receipt remain unchanged. This copy only
normalizes NumPy scalars for JSON and saves a separately named complete receipt.
"""
...
def json_scalars(value):
    ...
    if isinstance(value, (list, tuple)): return [json_scalars(item) for item in value]
...
def main():
    source, target = R/'evaluation_summary.json', R/'qa_evaluation_v31_completed.json'
    ...
    receipt['serialization_repair'] = dict(calculations_changed=False,
        reason='Original frozen QA passed all assertions but numpy.bool_ could not be serialized.',
        original_frozen_QA=str(Path(__file__).with_name('qa_evaluation_v31.py')),
        original_frozen_QA_sha256=sha(Path(__file__).with_name('qa_evaluation_v31.py')),
        preserved_incomplete_receipt=str(R/'qa_evaluation_v31.json'),
        preserved_incomplete_receipt_sha256=sha(R/'qa_evaluation_v31.json'))
    receipt = json_scalars(receipt)
