#!/usr/bin/env python3
"""Offline multi-ROI rPPG with explicit ambiguity and forward HR decisions.
...
Filtering and overlap fusion are offline; forward HR state is not an online claim.
"""
...
def source_hashes():
    names=['analyze_motion_v2.py','analyze_rppg.py','legacy_motion.py','motion_frontend.py','motion_fusion.py']
...
def main():
    ...
    config=FusionConfig()
    fused,wave,diagnostics=fuse_windows(signals,trace,fps,args.window,args.step,args.min_bpm,args.max_bpm,
                                       config=config)
    fused['waveform_generated']=fused.accepted & (fused.waveform_roi_count>=config.min_rois)
    tables['fusion']=fused; waveforms['fusion']=wave
    pd.DataFrame(dict(time_s=trace.time_s,base=wave)).to_csv(args.output/'fusion_waveform.csv',index=False)
    diagnostics.to_csv(args.output/'fusion_diagnostics.csv',index=False)
    variants={}
    ...
    summary=dict(version='multi_roi_v2',identity=identity,source_hashes=frozen_sources,fps=fps,frames=len(trace),
                 ...
                 limitations=['Waveforms use offline zero-phase filtering and overlap fusion.',
                              'Fusion HR decisions are forward-only but the full pipeline is NOT real-time.',
                              'accepted and quality_proxy are engineering decisions, not calibrated accuracy probabilities.',
    ...
    axes[0].set(ylabel='Sampling',ylim=(-.1,1.2),title='V2 multi-ROI: sampling and candidate HR (not an accuracy claim)')
