"""Frozen robust detection/tracking with three separately retained RGB patches.
...
RGB filling here. ``r/g/b`` remain the unweighted mean of valid patch medians.
...
def _empty_region(status):
    return dict(r=np.nan, g=np.nan, b=np.nan, valid=False, quality=0.0,
                ...
                laplacian_var=np.nan, status=status)


def _empty_samples(status):
    row = dict(rgb_valid=False, r=np.nan, g=np.nan, b=np.nan,
               roi_status=status, roi_valid_regions=0)
    for name in REGION_NAMES:
        row.update({f"{name}_{key}": value for key, value in _empty_region(status).items()})
    return row


def sample_rois(frame_rgb, landmarks):
    """Return merged legacy RGB plus independent patch medians/diagnostics.
    ...
    if landmarks is None:
        return _empty_samples("no_face_or_track")
    ...
    if points.ndim != 2 or points.shape[0] < 2 or not np.isfinite(points).all():
        return _empty_samples("invalid_face_geometry")
    x0, x1 = np.percentile(points[:, 0], [2, 98])
    y0, y1 = np.percentile(points[:, 1], [2, 98])
    fw, fh = x1 - x0, y1 - y0
    if fw < 40 or fh < 40:
        return _empty_samples("face_too_small")

    result = _empty_samples("no_valid_skin_patch")
    medians = []
    for name, (rx0, ry0, rx1, ry1) in REGION_BOXES.items():
        ...
        info = _empty_region("outside_image")
        if xa < xb and ya < yb:
            ...
            if len(pixels) >= 25:
                ...
                info.update(r=float(median[0]), g=float(median[1]), b=float(median[2]),
    ...
    if medians:
        value = np.mean(medians, axis=0)
        result.update(rgb_valid=True, r=float(value[0]), g=float(value[1]), b=float(value[2]),
                      roi_status="sampled" if len(medians) == len(REGION_NAMES) else "sampled_partial_regions",
                      roi_valid_regions=len(medians))
    return result


def extract(video, max_seconds=None, qa_dir=None):
    ...
    try:
        while index < limit:
            ...
            sampling = _empty_samples("bridge_limit_reached" if prediction is not None
                                      else "no_face_and_no_valid_flow")
            if points is not None:
                full = points * np.array([w / sw, h / sh])
                sampling = sample_rois(rgb, legacy.mesh_object(full, w, h))
