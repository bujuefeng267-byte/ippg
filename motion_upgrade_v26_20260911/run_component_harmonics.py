...
import numpy as np
import pandas as pd
from component_harmonics_v26 import fuse_components,ComponentConfig,ROIS
from evidence_hr import estimate_evidence
from analyze_motion_v2 import validate_trace,source_hashes

HERE=Path(__file__).resolve().parent
P=Path('/home/fengbujue/项目/rppg识别');BASE=P/'results/data1_6_20260911';ROOT=P/'results/data1_6_v26_20260911';OUT=ROOT/'component_harmonics'
def sha(p):return hashlib.sha256(Path(p).read_bytes()).hexdigest()
def save(p,x):p.write_text(json.dumps(x,ensure_ascii=False,indent=2,allow_nan=False))
def frozen():return {**source_hashes(),**{n:sha(HERE/n) for n in ['component_harmonics_v26.py','run_component_harmonics.py','motion_harmonic_evidence_v26.py','test_motion_harmonic_evidence_v26.py','test_component_fusion_v26.py','evaluate_v26.py']}}
...
protocol=dict(created_utc=datetime.now(timezone.utc).isoformat(),source_hashes=fixed,component_config=asdict(ComponentConfig()),
    motion_harmonic_rule='max(profile(f), 0.8*profile(2f), 0.5*profile(f/2)); one extended-spectrum normalization, absolute strength and reliability retained; unavailable frequencies unsupported',
    reference_used=False,post_diagnosis_development=True,
    reason='Post-component development test: penalize candidate frequency with fixed direct, double-frequency and half-frequency geometric-motion evidence; preserve all other joint component inference and final V25 waveform readout.',
    ...
    previously_observed_development_reports={name:sha(ROOT/name/'evaluation_summary.json') for name in ['log_projection_baseline','log_projection_guarded','component_consensus','neural_efficientphys']},
...
for item in inputs:
    ...
    summary=dict(case=case,variant='component_harmonics',fps=fps,frames=len(trace),source_hashes=fixed,
