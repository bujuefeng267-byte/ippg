"""Conservative, reference-free routing between two saved real waveforms.

This final bounded V26 candidate leaves the V25 waveform untouched unless a
complete 10 s window has an accepted old HR and a generated multi-ROI component
...
is an input. Harmonic motion evidence is a cue, not proof of an artifact.
...
def _plan_table(table, centers, starts, window, fps, name):
    if len(table) != len(centers) or not np.allclose(
        ...
        raise ValueError(f'{name} must match the complete ordered 10 s / 1 s window plan')
...
def route_components(old_wave, old_hr, candidate_wave, proposals, trace, fps):
    ...
    window, step = round(WINDOW_SECONDS * fps), round(STEP_SECONDS * fps)
