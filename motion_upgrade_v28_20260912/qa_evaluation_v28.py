...
ROOT = PROJECT/'results/data1_6_v28_20260912'
R26 = PROJECT/'results/data1_6_v26_20260911'
...
def coverage_totals(rows):
    all_seconds = sum(r['waveform_frames']/r['fps'] for r in rows)
    finite_seconds = sum(r['waveform_finite_frames']/r['fps'] for r in rows)
    return dict(Nplanned=sum(r['Nplanned'] for r in rows), Noutput=sum(r['Noutput'] for r in rows),
        waveform_frame_coverage_pct=100*sum(r['waveform_finite_frames'] for r in rows)/sum(r['waveform_frames'] for r in rows),
        waveform_time_coverage_pct=100*finite_seconds/all_seconds)


def independently_pool_without_common(rows):
    n, nr = sum(r['Nvalid'] for r in rows), sum(r['Nref'] for r in rows)
    n5 = sum(r['Nwithin5'] for r in rows)
    return dict(**coverage_totals(rows), Nref=nr, Nvalid=n, Nwithin5=n5,
...
def main():
    ...
    parser.add_argument('--variants', nargs='+', choices=['direct_guard'], default=['direct_guard'])
    parser.add_argument('--output', type=Path, default=ROOT/'qa_evaluation_v28.json')
    ...
    for variant in args.variants:
        ...
        rows = []
        comparator_rows = {'V25': [], 'V26_harmonic': [], 'V26_conservative': []}
        ...
        for i in range(1, 7):
            ...
            row = dict(**m, fps=fps, waveform_frames=nframes, waveform_finite_frames=int(finite.sum()),
                V25_MAE_bpm=oldm['MAE_bpm'], delta_MAE_bpm=m['MAE_bpm']-oldm['MAE_bpm'],
            ...
            decisions = pd.read_csv(folder/'routing_decisions.csv')
            assert saved['routed_windows'] == int(bools(decisions.route_eligible).sum())
            assert saved['routing_reason_counts'] == decisions.reason.value_counts().to_dict()
            ...
            for name, baseline in [('V25', V25), ('V26_harmonic', R26/'component_harmonics'),
                                   ('V26_conservative', R26/'conservative_components')]:
                hr_file = baseline/case/('fusion_heart_rate.csv' if name == 'V25' else 'heart_rate.csv')
                wave_file = baseline/case/('fusion_waveform.csv' if name == 'V25' else 'waveform.csv')
                bh, bw = pd.read_csv(hr_file), pd.read_csv(wave_file)
                bind(hr_file); bind(wave_file)
                by, bok = bh.ridge_bpm.to_numpy(float), bools(bh.accepted)
                bf = np.isfinite(bw.base)
                bm = dict(case=case, **metrics(by, bok, r), fps=fps, waveform_frames=nframes,
                    waveform_finite_frames=int(bf.sum()), waveform_coverage_pct=100*bf.mean())
                comparator_rows[name].append(bm)
                compare_dict(next(x for x in report['baselines'][name]['cases'] if x['case'] == case), bm,
                             variant+'/'+case+'/'+name)
                bm_common, bm_new, bm_lost = ok & bok & eligible, ok & ~bok & eligible, ~ok & bok & eligible
                difference = dict(case=case, comparator=name,
                    delta_MAE_bpm=m['MAE_bpm']-bm['MAE_bpm'], delta_RMSE_bpm=m['RMSE_bpm']-bm['RMSE_bpm'],
                    delta_P5_pp=m['P5_valid_pct']-bm['P5_valid_pct'],
                    delta_R5_pp=m['R5_all_reference_pct']-bm['R5_all_reference_pct'],
                    delta_hr_coverage_pp=m['hr_output_coverage_pct']-bm['hr_output_coverage_pct'],
                    delta_waveform_coverage_pp=100*(finite.mean()-bf.mean()),
                    common_windows=int(bm_common.sum()), new_windows=int(bm_new.sum()), lost_windows=int(bm_lost.sum()),
                    common_MAE_bpm=float(abs(y[bm_common]-r[bm_common]).mean()) if bm_common.any() else np.nan,
                    common_comparator_MAE_bpm=float(abs(by[bm_common]-r[bm_common]).mean()) if bm_common.any() else np.nan,
                    new_MAE_bpm=float(abs(y[bm_new]-r[bm_new]).mean()) if bm_new.any() else np.nan,
                    lost_comparator_MAE_bpm=float(abs(by[bm_lost]-r[bm_lost]).mean()) if bm_lost.any() else np.nan)
                independent_differences.append(difference)
                saved_difference = next(x for x in report['comparator_differences'] if x['case'] == case and x['comparator'] == name)
                compare_dict(saved_difference, difference, variant+'/'+case+'/'+name+'/difference')
            rows.append(row)
            ...
            for path in paths:
                bind(path)
        agg = pooled(rows)
        agg.update(coverage_totals(rows))
        assert agg['Nplanned'] == agg['Nref'] == 309
        compare_dict(report['pooled'], agg, variant+'/pooled')
        for name, baseline_rows_ in comparator_rows.items():
            baseline_pooled = independently_pool_without_common(baseline_rows_)
            compare_dict(report['baselines'][name]['pooled'], baseline_pooled, variant+'/'+name+'/pooled')
            differences_ = [x for x in independent_differences if x['comparator'] == name]
            nc_ = sum(x['common_windows'] for x in differences_)
            pooled_difference = dict(**{'delta_'+key: agg[key]-baseline_pooled[key] for key in (
                'MAE_bpm','RMSE_bpm','P5_valid_pct','R5_all_reference_pct','HR_coverage_pct','waveform_time_coverage_pct')},
                common_windows=nc_,
                common_MAE_bpm=sum(x['common_windows']*x['common_MAE_bpm'] for x in differences_ if x['common_windows'])/nc_ if nc_ else np.nan,
                common_comparator_MAE_bpm=sum(x['common_windows']*x['common_comparator_MAE_bpm'] for x in differences_ if x['common_windows'])/nc_ if nc_ else np.nan)
            compare_dict(report['pooled_comparator_differences'][name], pooled_difference, variant+'/'+name+'/pooled_difference')
    ...
    result = dict(created_utc=datetime.now(timezone.utc).isoformat(),passed=not errors, errors=errors,
        ...
        evidence_timing='Independent QA hashes are supplemental; original evaluation inputs are separately prospectively bound in V28 protocol_before_run.json.',
        review=['Independent arithmetic implementation; no import of evaluate_v28, reference_core, or production models.',
