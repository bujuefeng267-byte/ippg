"""Auditable multi-ROI waveform proposals and forward proposal gating.

Input signals are already filtered/edge-trimmed offline by the frozen frontend.
No reference HR/PPG is accepted or read. ``ridge_bpm`` is an internal compatibility
name for a forward continuity-gated proposal, NOT the final reported HR.
waveform_hr.py computes final local/DP HR from the actual stitched waveform.
Each proposal uses its complete window and past decisions; it is only available
...
rejected proposal rows are always NaN, never held or interpolated.
"""
...
class FusionConfig:
    ...
    motion_evidence_mode: str = 'legacy'
    reliable_motion_penalty: float = 0.75
    ...
    def __post_init__(self):
        ...
        if self.motion_evidence_mode not in ('legacy','reliable'):
            raise ValueError('Unknown motion evidence mode')
        if not np.isfinite(self.reliable_motion_penalty) or not 0 <= self.reliable_motion_penalty < 1:
            raise ValueError('Reliable motion penalty must remain finite and soft (<1)')
...
MOTION_COLUMNS = ['motion_available','motion_observed_fraction','motion_speed_face_per_s',
                  'motion_strength','motion_reliability','motion_spectral_coverage','motion_evidence_status']
...
def _groups(candidates, config):
    """Stable membership and frequency, independent of candidate traversal."""
    return group_candidates(candidates, config)
...
def fuse_windows(signals: dict[str, np.ndarray], trace: pd.DataFrame, fps: float,
                 ...
                 config: FusionConfig | None = None, return_segments=False):
    """Return ``(proposal table, actual fused waveform, candidate diagnostics)``.
    ...
    for wi, start in enumerate(range(0, n - window + 1, step)):
        ...
        if config.motion_evidence_mode == 'reliable':
            evidence = motion_evidence(trace, start, stop, fps, grid)
            motion = evidence['profile']
            motion_meta = dict(motion_available=evidence['available'],
                motion_observed_fraction=evidence['observed_fraction'],
                motion_speed_face_per_s=evidence['speed_face_per_s'],
                motion_strength=evidence['strength'],motion_reliability=evidence['reliability'],
                motion_spectral_coverage=evidence['spectral_coverage'],
                motion_evidence_status=evidence['status'])
            penalty = config.reliable_motion_penalty
        else:
            motion = _motion_profile(trace, start, stop, fps, grid, config)
            motion_meta = dict(motion_available=False,motion_observed_fraction=np.nan,
                motion_speed_face_per_s=np.nan,motion_strength=np.nan,motion_reliability=np.nan,
                motion_spectral_coverage=np.nan,motion_evidence_status='legacy_not_assessed')
            penalty = config.max_motion_penalty
        ...
        for channel, (roi, method) in expected.items():
            ...
            base = dict(window_index=wi, time_s=center, channel=channel, roi=roi,
                        ...
                        output_accepted=False, waveform_weight=0.0, **motion_meta)
            for rank, peak in enumerate(peaks or [None], 1):
                ...
                if peak is not None:
                    ...
                    d["motion_weight"] = 1 - penalty * d["motion_overlap"]
        ...
        row = dict(time_s=center, window_start_s=start / fps, window_end_s=stop / fps,
                   ...
                   candidate_group_count=len(groups), waveform_roi_count=0, **motion_meta)
        if best is not None:
            # Reliable mode compares the same evidence-adjusted scores used for
            # ranking. Raw scores would declare a resolved motion competitor
            # ambiguous even when its score was deliberately downweighted.
            ratio_key = 'score' if config.motion_evidence_mode == 'reliable' else 'base_score'
            ratio = max((g[ratio_key] / best[ratio_key] for g in groups[1:]), default=0.0)
        ...
        if row["accepted"]:
            ...
            if fused is not None:
                if return_segments:
                    segments[wi] = dict(waveform=fused.copy(), quality=best['quality'],
                                        used=used.copy())
    ...
    return (table, waveform, diag, segments) if return_segments else (table, waveform, diag)
