"""Assemble fixed six-case development comparison from audited saved metrics."""
...
STAGES=['v24_replay','stage1_motion','stage2_hr','stage3_routing','stage4_preserve_waveform']
LABELS=['V24 原版回放','① 运动证据进入融合','② 加上多候选 HR','③ 再加新分支选择','采用：保留原波形 + 新 HR']

def main():
    ap=argparse.ArgumentParser();ap.add_argument('--package',action='store_true');args=ap.parse_args()
    reports=[json.loads((ROOT/s/'stage_evaluation.json').read_text()) for s in STAGES]
    old,new=reports[0],reports[-1];assert new['adoption_pass']
    o,n=old['pooled'],new['pooled']; reduction=100*(1-n['MAE_bpm']/o['MAE_bpm'])
    lines=['# V2.5 代码优化与六视频指标变化', '',
        '**已按运动频率判别、多候选心率与重获、分支选择的顺序修改并验证。推荐版采用新心率判断，保留原来的实际波形融合与分支选择；新融合和新路由仍可单独实验。**', '',
        f"六组有效配对窗口的合并 MAE：**{o['MAE_bpm']:.2f} → {n['MAE_bpm']:.2f} bpm（降低 {reduction:.2f}%）**；全参考窗口 ±5 bpm 成功率 R5：**{o['R5_all_reference_pct']:.2f}% → {n['R5_all_reference_pct']:.2f}%（+{n['R5_all_reference_pct']-o['R5_all_reference_pct']:.2f} 个百分点）**。PPG 波形覆盖和 HR 输出覆盖保持不变，逐样本波形在数值精度内一致（复算容差 1e-8）。", '',
    ...
    for a,b in zip(old['cases'],new['cases']):
        vals=[f"{a[k]:.2f} → {b[k]:.2f}"+('%' if k.endswith('pct') else '') for k in ['MAE_bpm','RMSE_bpm','P5_valid_pct','R5_all_reference_pct']]
        lines.append('| '+b['case']+' | '+' | '.join(vals)+' |')
    lines+=['', '| 视频 | 波形覆盖率（前后相同） | HR 覆盖率（前后相同） | 输出/计划窗（前后相同） | ±5 bpm 窗数 |',
        '|---|---:|---:|---:|---:|']
    for a,b in zip(old['cases'],new['cases']):
        lines.append(f"| {b['case']} | {b['waveform_coverage_pct']:.2f}% | {b['hr_output_coverage_pct']:.2f}% | {b['Noutput']}/{b['Nplanned']} | {a['Nwithin5']} → {b['Nwithin5']} |")
    lines+=['', f"合并结果：309 个计划/合格参考窗口，260 个有效配对输出，±5 bpm 窗数 {o['Nwithin5']} → {n['Nwithin5']}；P5 {100*o['Nwithin5']/o['Nvalid']:.2f}% → {100*n['Nwithin5']/n['Nvalid']:.2f}%；合并 RMSE {o['RMSE_bpm']:.2f} → {n['RMSE_bpm']:.2f} bpm。新旧是相同的 260 个窗口，因此推荐版的 MAE 降低不是丢掉困难时段造成的。", '',
        ...
        '|---|---:|---:|---:|---|']
    for label,s,r in zip(LABELS,STAGES,reports):
        p=r['pooled'];decision='通过' if r['adoption_pass'] else ('对照' if s=='v24_replay' else '未通过')
        lines.append(f"| [{label}]({s}/stage_metrics.csv) | {p['MAE_bpm']:.2f} | {p['R5_all_reference_pct']:.2f}% | {p['hr_output_coverage_pct']:.2f}% | {decision} |")
    lines+=['',
        ...
        '|---|---|---|---|---|']
    for b in new['cases']:
        c=b['case'];p=f'stage4_preserve_waveform/{c}'
        lines.append(f'| {c} | [PNG]({p}/presentation/ppg_hr.png) / [SVG]({p}/presentation/ppg_hr.svg) | [CSV]({p}/fusion_waveform.csv) | [CSV]({p}/fusion_heart_rate.csv) | [CSV]({p}/evaluation/paired_windows.csv) |')
    lines+=['', '![六组保存波形及新旧心率](presentation/all_six_ppg_hr.png)', '',
        ...
        '|---|---:|---:|']
    for b in new['cases']:
        c=b['case'];sens=pd.read_csv(ROOT/'stage4_preserve_waveform'/c/'evaluation/alignment_sensitivity.csv')
        lines.append(f"| {c} | {b['MAE_bpm']:.2f} | {sens.MAE_bpm.min():.2f}–{sens.MAE_bpm.max():.2f} |")
    lines+=['', '[独立指标与协议审计](qa_protocol_integrity_final.json) · [保存波形与 HR 证据审计](qa_v25_evidence.json)', '',
        ...
        '```bash', 'cd /home/fengbujue/项目/rppg识别', './run_motion_v25.sh videos/your_video.avi --output results/your_video_v25', '```', '',
    ...
    (ROOT/'V25代码优化与六视频指标变化.md').write_text('\n'.join(lines)+'\n',encoding='utf-8')
    pd.concat([pd.read_csv(ROOT/s/'stage_metrics.csv') for s in STAGES],ignore_index=True).to_csv(ROOT/'all_stages_metrics.csv',index=False)
    if args.package:
        paths=[ROOT/'V25代码优化与六视频指标变化.md',ROOT/'all_stages_metrics.csv',ROOT/'protocol_before_validation.json',ROOT/'qa_protocol_integrity_final.json',ROOT/'qa_v25_evidence.json']
        paths+=list((ROOT/'presentation').glob('*'))
        for s in STAGES:paths += [ROOT/s/'stage_metrics.csv',ROOT/s/'stage_evaluation.json']
        paths += [ROOT/'stage4_preserve_waveform/protocol_before_run.json']
        for b in new['cases']:
            d=ROOT/'stage4_preserve_waveform'/b['case']
            paths += [d/x for x in ('fusion_waveform.csv','fusion_heart_rate.csv','summary.json','evaluation/paired_windows.csv','evaluation/metrics.json','evaluation/alignment_sensitivity.csv')]
            paths += list((d/'presentation').glob('*'))
        with zipfile.ZipFile(ROOT/'V25六视频结果与对比.zip','w',zipfile.ZIP_DEFLATED) as z:
            for p in paths:
                assert p.is_file(),p
                z.write(p,p.relative_to(ROOT))
            z.write(Path(__file__).with_name('README_V25.md'),'代码运行说明.md')
        with zipfile.ZipFile(ROOT/'V25六视频结果与对比.zip') as z:assert z.testzip() is None
    print(json.dumps(dict(report=str(ROOT/'V25代码优化与六视频指标变化.md'),MAE_reduction_pct=reduction,package=args.package),ensure_ascii=False))
