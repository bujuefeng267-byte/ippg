#!/usr/bin/env python3
"""V2.5 staged motion evidence, saved-waveform HR and branch routing.
...
from pixel_tracking import MODES, PixelConfig
...
def source_hashes():
    names=['analyze_motion_v2.py','analyze_rppg.py','legacy_motion.py','motion_frontend.py',
           'motion_fusion.py','stable_groups.py','waveform_hr.py','baseline_frontend.py','pixel_tracking.py',
           'anchored_reconstruction.py','guarded_fusion.py','motion_evidence.py','evidence_hr.py']
...
def main():
    ...
    ap.add_argument('--geometry-cache',type=Path,help='Frozen V2.2 trace; original video is reread for pixel processing')
    ap.add_argument('--tracking-cache',type=Path,help='Verified V2.3 screened paired increments; reconstruction is recomputed')
    ap.add_argument('--algorithm-mode',choices=['legacy','bounded_tracking','guarded_fusion'],default='legacy')
    ap.add_argument('--pixel-mode',choices=MODES,default='disabled',help='Explicit experimental paired-pixel mode')
    ...
    ap.add_argument('--motion-evidence',choices=['legacy','reliable'],default='legacy')
    ap.add_argument('--hr-mode',choices=['legacy','evidence'],default='legacy')
    ap.add_argument('--routing-mode',choices=['legacy','evidence'],default='legacy')
    ap.add_argument('--reference-ubfc',type=Path)
    args=ap.parse_args()
    if sum(x is not None for x in (args.cache,args.geometry_cache,args.tracking_cache))>1:
        ap.error('Use at most one cache input')
    if args.algorithm_mode!='legacy' and args.pixel_mode!='tracking_screened':
        ap.error('V2.4 algorithms require all three tracking/screening/fallback steps: tracking_screened')
    if args.tracking_cache and (args.algorithm_mode=='legacy' or args.pixel_mode!='tracking_screened'):
        ap.error('Tracking-cache is only for V2.4 screened reconstruction')
    ...
    identity=dict(video=str(args.video.resolve()),size=stat.st_size,mtime_ns=stat.st_mtime_ns,
                  ...
                  pixel_mode=args.pixel_mode,pixel_config=asdict(PixelConfig()),
                  rgb_reconstruction='v23_integral' if args.algorithm_mode=='legacy' else 'anchored_log',
                  anchor_config=asdict(AnchorConfig()) if args.algorithm_mode!='legacy' else None,
                  frontend_hashes={n:digest(HERE/n) for n in ['motion_frontend.py','baseline_frontend.py',
                      'pixel_tracking.py','legacy_motion.py','analyze_rppg.py','anchored_reconstruction.py']})
    ...
    elif args.tracking_cache:
        metadata=json.loads(args.tracking_cache.with_suffix('.json').read_text(encoding='utf-8'))
        old_id=metadata['identity']
        for key in ('video','size','mtime_ns','max_seconds','rgb_aggregation','pixel_mode','pixel_config'):
            if old_id.get(key)!=identity[key]:raise ValueError(f'Tracking cache identity mismatch: {key}')
        expected={n:digest(HERE/n) for n in ['motion_frontend.py','baseline_frontend.py','pixel_tracking.py',
                                          'legacy_motion.py','analyze_rppg.py']}
        if old_id.get('frontend_hashes')!=expected or metadata['trace_sha256']!=digest(args.tracking_cache):
            raise ValueError('V23 paired-tracking source/content mismatch')
        trace=pd.read_csv(args.tracking_cache);fps=metadata['fps']
        if len(trace)!=metadata['n_frames']:raise ValueError('Tracking cache frame count mismatch')
        validate_trace(trace,fps)
        trace=anchored_reconstruction(trace,fps)
    else:
        geometry_trace,geometry_fps=None,None
        if args.geometry_cache:
            meta=json.loads(args.geometry_cache.with_suffix('.json').read_text(encoding='utf-8'))
            old_id=meta['identity']
            for key in ('video','size','mtime_ns','max_seconds','rgb_aggregation'):
                if old_id.get(key)!=identity[key]:
                    raise ValueError(f'Baseline geometry cache identity mismatch: {key}')
            expected={'motion_frontend.py':digest(HERE/'baseline_frontend.py'),
                      'legacy_motion.py':digest(HERE/'legacy_motion.py'),
                      'analyze_rppg.py':digest(HERE/'analyze_rppg.py')}
            if old_id['frontend_hashes']!=expected or meta['trace_sha256']!=digest(args.geometry_cache):
                raise ValueError('Baseline geometry cache code/content checksum mismatch')
            geometry_trace=pd.read_csv(args.geometry_cache);geometry_fps=meta['fps']
            if len(geometry_trace)!=meta['n_frames']:raise ValueError('Baseline frame count mismatch')
            validate_trace(geometry_trace,geometry_fps)
        trace,fps=extract(args.video,max_seconds=args.max_seconds,qa_dir=args.output/'qa',
                          rgb_aggregation=args.rgb_aggregation,pixel_mode=args.pixel_mode,
                          geometry_trace=geometry_trace,geometry_fps=geometry_fps)
        if args.algorithm_mode!='legacy':trace=anchored_reconstruction(trace,fps)
    validate_trace(trace,fps)
    ...
    config=FusionConfig(motion_evidence_mode=args.motion_evidence)
    guard_config=GuardConfig(routing_mode=args.routing_mode)
    if args.algorithm_mode=='guarded_fusion':
        original=baseline_view(trace)
        original_signals,original_roi_wave=per_roi_signals(original,fps,args.max_gap,args.min_bpm,args.max_bpm)
        original_roi_wave.to_csv(args.output/'baseline_roi_waveforms.csv',index=False)
        # Both branches have identical observation/interpolation masks. A branch
        # switch cannot turn imputed data into observed pixels.
        for roi in ROIS:
            for tag in ['observed','interpolated']:
                if not np.array_equal(roi_wave[f'{roi}_{tag}'],original_roi_wave[f'{roi}_{tag}']):
                    raise RuntimeError('Branch sampling provenance differs')
        proposals,wave,diagnostics,routing,branches=guarded_fuse(original_signals,signals,trace,fps,
            args.window,args.step,args.min_bpm,args.max_bpm,config=config,guard=guard_config)
        routing.to_csv(args.output/'branch_routing.csv',index=False)
        for branch,(p,w,d,_) in branches.items():
            p.to_csv(args.output/f'{branch}_branch_proposals.csv',index=False)
            pd.DataFrame({'time_s':trace.time_s,'base':w}).to_csv(args.output/f'{branch}_branch_waveform.csv',index=False)
    else:
    ...
    fused,provenance=estimate_fused_waveform(wave,trace,roi_wave,proposals,diagnostics,fps,
        args.window,args.step,args.min_bpm,args.max_bpm,hr_mode=args.hr_mode)
    ...
    summary=dict(version='multi_roi_v2.5_experimental',identity=identity,source_hashes=frozen_sources,fps=fps,frames=len(trace),
    ...
    summary['pixel_config']=asdict(PixelConfig())
    summary['anchor_config']=asdict(AnchorConfig()) if args.algorithm_mode!='legacy' else None
    summary['guard_config']=asdict(guard_config) if args.algorithm_mode=='guarded_fusion' else None
    if args.algorithm_mode=='guarded_fusion':
        summary['branch_selection_counts']={str(k):int(v) for k,v in routing.selected_branch.value_counts().items()}
    summary['extraction_path']=('validated_V23_paired_increments' if args.tracking_cache else
        'validated_V24_frontend_cache' if args.cache else 'fresh_pixels_with_V22_geometry' if args.geometry_cache else
        'fresh_detector_and_pixels')
    summary['opencv_threads']=cv2.getNumThreads()
    if args.pixel_mode!='disabled':
        summary['pixel_diagnostics']={roi:{str(k):int(v) for k,v in trace[f'{roi}_pixel_source'].value_counts().items()}
                                      for roi in ROIS}
        summary['limitations'] += ['Candidate RGB is paired relative-color reconstruction, not raw RGB.',
            'Slow anchoring bounds integration drift but admits some original motion at pulse frequencies.',
            'Guard scores are relative engineering evidence, not calibrated accuracy probabilities.',
            'Tracking fallback preserves baseline sampling availability; it does not establish pulse quality.',
            'Coherent motion or lighting can survive screening and produce non-physiological spectral peaks.']
    ...
    axes[0].set(ylabel='Sampling',ylim=(-.1,1.2),title=f'V2.5 {args.motion_evidence}/{args.hr_mode}/{args.routing_mode}: sampling and offline HR')
