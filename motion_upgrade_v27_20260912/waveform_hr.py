...
"""
    ...
    if hr_mode not in ('legacy', 'evidence'):
        raise ValueError('hr_mode must be legacy or evidence')
    n = len(trace)
    ...
    if hr_mode == 'evidence':
        ...
        table = estimate_evidence(actual, pd.DataFrame({'rgb_valid': observed}),
            interpolated, fps, window_s, step_s, min_bpm, max_bpm, motion_trace=trace)
    else:
    ...
    table.attrs.update(hr_tracker=('offline_evidence_DP_on_actual_fused_waveform'
                       if hr_mode == 'evidence' else 'offline_DP_on_actual_fused_waveform'),
