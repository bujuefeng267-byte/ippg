#!/usr/bin/env python3
"""V2.3 experimental paired-pixel motion compensation with frozen V2.2 backend.
...
def main():
    ...
    if args.cache and args.geometry_cache:
        ap.error('Use either candidate cache or baseline geometry cache')
    ...
    identity=dict(video=str(args.video.resolve()),size=stat.st_size,mtime_ns=stat.st_mtime_ns,
                  ...
                  frontend_hashes={n:digest(HERE/n) for n in ['motion_frontend.py','baseline_frontend.py',
                      'pixel_tracking.py','legacy_motion.py','analyze_rppg.py']})
    ...
    summary=dict(version='multi_roi_v2.3_experimental',identity=identity,source_hashes=frozen_sources,fps=fps,frames=len(trace),
    ...
    axes[0].set(ylabel='Sampling',ylim=(-.1,1.2),title=f'V2.3 {args.pixel_mode}: sampling and offline HR')
